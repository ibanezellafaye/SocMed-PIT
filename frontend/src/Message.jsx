        ...
        <div className="overflow-y-auto">
          {conversations.map(user => (
            <div
              key={user.id}
              onClick={() => handleUserClick(user)}
              className={`flex items-center p-2 cursor-pointer ${selectedUser && selectedUser.id === user.id ? 'bg-blue-500 text-white' : ''} ${theme === 'dark' ? 'hover:bg-gray-700' : 'hover:bg-gray-200'}`}
            >
              {user.profile_image_url ? (
                <img src={user.profile_image_url} alt="Profile" className="w-8 h-8 rounded-full mr-2" />
              ) : (
                <div className="w-8 h-8 rounded-full bg-gray-700 flex items-center justify-center text-gray-500 mr-2">
                  No Image
                </div>
              )}
              <div>
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="flex-grow overflow-y-auto p-4">
              {messages.map((message, index) => (
                <div key={index} className={`mb-2 ${message.sender_id === currentUser.id ? 'text-right' : 'text-left'}`}>
                  <div className={`p-2 rounded-md ${message.sender_id === currentUser.id ? 'bg-blue-500 text-white' : theme === 'dark' ? 'bg-gray-700' : 'bg-gray-200'}`}>
                    <p className="text-sm font-semibold">{message.sender_id === currentUser.id ? 'You' : `${selectedUser.first_name} ${selectedUser.last_name}`}</p>
                    <p className="text-xs text-gray-400">{moment(message.created_at).format('MMMM Do YYYY, h:mm:ss a')}</p>
                    <p>{message.content}</p>
                  </div>
                </div>
