...
const Header = ({ onLogout }) => {
    ...
    <div className={`fixed w-full flex items-center justify-between p-4 shadow-md ${theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-white text-black'}`}>
      ...
      <form onSubmit={handleSearch} className="flex items-center relative flex-grow max-w-xl ml-96">
        <input
          ...
          className="pl-8 pr-32 w-[28rem] py-2 border rounded-lg flex items-center justify-between "
        />
        <button type="submit" className="py-2 px-4 bg-blue-600 hover:bg-blue-700 rounded-full transition duration-200 text-white ml-2">
          Search
        </button>
      </form>
      <div className="flex space-x-4 ml-4 relative">
        <button
          ...
          className="relative py-2 px-4  rounded-full  transition duration-200 text-white bg-gray-200 hover:bg-gray-300 active:bg-gray-500 "
        ><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6 fill-gray-500 rounded-full active:fill-gray-500">
        <path d="M5.85 3.5a.75.75 0 0 0-1.117-1 9.719 9.719 0 0 0-2.348 4.876.75.75 0 0 0 1.479.248A8.219 8.219 0 0 1 5.85 3.5ZM19.267 2.5a.75.75 0 1 0-1.118 1 8.22 8.22 0 0 1 1.987 4.124.75.75 0 0 0 1.48-.248A9.72 9.72 0 0 0 19.266 2.5Z" />
        <path fillRule="evenodd" d="M12 2.25A6.75 6.75 0 0 0 5.25 9v.75a8.217 8.217 0 0 1-2.119 5.52.75.75 0 0 0 .298 1.206c1.544.57 3.16.99 4.831 1.243a3.75 3.75 0 1 0 7.48 0 24.583 24.583 0 0 0 4.83-1.244.75.75 0 0 0 .298-1.205 8.217 8.217 0 0 1-2.118-5.52V9A6.75 6.75 0 0 0 12 2.25ZM9.75 18c0-.034 0-.067.002-.1a25.05 25.05 0 0 0 4.496 0l.002.1a2.25 2.25 0 1 1-4.5 0Z" clipRule="evenodd" />
      </svg>
       {unreadCount > 0 && <span className="absolute bottom-auto left-auto right-0 top-0 z-10 inline-block -translate-y-1/2 translate-x-2/4 rotate-0 skew-x-0 skew-y-0 scale-x-100 scale-y-100 rounded-full bg-red-700 p-2 text-xs">{unreadCount}</span>}
        </button>
        {showNotifications && (
 ...
 : <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6 hover:fill-indigo-900">
