...
const linkClasses = 'rounded-lg flex items-center gap-2 font-light px-3 py-2 hover:bg-cyan-300 hover:no-underline active:bg-cyan-500 text-base';
        ...
        <aside className="h-screen">
            <nav className="h-full flex flex-col bg-white border-r shadow-sm rounded-lg">
                ...
                <SidebarContext.Provider value={{ expanded }}>
                    ...
                    <ul className="flex-1 px-3 gap-0.5 rounded-lg">
...
function SidebarItem({ item }) {
    ...
    return (
        <li className={classNames(
            pathname === item.path ? 'bg-cyan-500 text-white rounded-lg' : 'text-black rounded-lg',
            linkClasses,
            "relative flex items-center py-2 px-3 my-1 font-medium rounded-lg cursor-pointer transition-colors group"
        )}>
             {/* Sidebar Hidden Hover */}
             <Link to={item.path} className="flex items-center gap-2 justify-items-center ml-2 rounded-lg ">
                {item.icon}
                <span className={`overflow-hidden transition-all justify-items-center rounded-lg ${expanded ? "w-52 ml-1" : "w-0"}`}>
                    {item.label}
                </span>
            </Link>
            {/* End */}
            {item.alert && (
                <div className={`absolute right-2 w-2 h-2 bg-cyan-400 rounded-lg ${expanded ? "" : "top-2"}`} />
            )}
            {!expanded && (
                <div className={`
...
function UserProfile({ user }) {
    return (
        <div>
            <div className="h-24 w-24 rounded-lg  bg-cover bg-no-repeat bg-center mx-auto">
