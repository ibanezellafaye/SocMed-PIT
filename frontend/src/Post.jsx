    ...
    <div className="post">
          ...
          {user && user.id === loggedInUser.id && (
            <>
              {isEditing ? (
                ...
                <>
                  <button onClick={handleEditPostContent} className="bg-yellow-500 text-white px-2 py-1 rounded ml-2">Edit</button>
                  <button onClick={onDelete} className="bg-red-500 text-white px-2 py-1 rounded ml-2">Delete</button>
                </>
              )}
            </>
      ...
      <div className="post-content mb-2">
        {editingContent ? (
          <div>
            <textarea
              className="w-full p-2 border border-gray-700 bg-gray-700 rounded mb-2"
              value={editedContent}
              onChange={(e) => setEditedContent(e.target.value)}
            />
            <button onClick={handleSavePostContent} className="bg-green-500 text-white px-2 py-1 rounded mr-2">Save</button>
      ...
      <div className="post-comments mt-4 mb-2">
        {Array.isArray(comments) && comments.slice(0, visibleComments).map((comment) => (
          <div key={comment.id} className="comment p-2 border-t flex items-center">
            {comment.user.profile_picture && (
              <img src={`http://localhost:8000/storage/${comment.user.profile_picture}`} alt="Profile" className="w-8 h-8 rounded-full mr-2" />
